...
- **10 个查询项目**（可勾选、可拖拽排序）：
  ...
  - 应缴费信息（导出四列，取第一条）
  - 应缴滞纳金信息（导出四列，取第一条）
- **两种查询模式**：按申请号直接查询，或按申请人搜索后批量查询该申请人的申请号
- **两种输入方式**：直接粘贴申请号/申请人，或上传 xlsx 文件（内置模板下载）
...
4. 选择“按申请号查询”或“按申请人查询”
5. 勾选要查询的字段（可拖动调整顺序）
6. 粘贴申请号/申请人（每行一个）或上传 xlsx 文件
7. 点“开始查询”，等待完成
8. 点“导出 Excel”下载结果
...
- **查询期间不要关闭页面**：脚本依赖当前页面的登录态；遇到临时风控时会自动新建 CNIPA 页面，自动恢复后在新页面继续抓取
- **登录态会过期**：如果提示"登录态过期"，在页面上重新搜索一次，然后点"重查失败项"
- **请求节流**：默认每次接口请求间隔约 1.2 秒，每 15 次接口请求自动冷却 20 秒；遇到临时风控时，先自动新建 CNIPA 标签页并触发一次页面查询，恢复后在新页面继续；新页面恢复失败时，当前申请号等待 45 秒自动重试，仍失败等待 30 秒后跳过，本轮完成后自动进行第二遍重查
