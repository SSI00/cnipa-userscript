...
(function () {
    ...
    const FIELDS = {
        ...
        feeAmount: {
            ...
            extract(data) {
                ...
                const value = list[0].yijiaofjfje;
    ...
    async function waitForApiSlot() {
        const targetInterval = API_REQUEST_MIN_INTERVAL_MS + Math.floor(
            Math.random() * (API_REQUEST_MAX_INTERVAL_MS - API_REQUEST_MIN_INTERVAL_MS + 1)
        );
        const intervalWait = targetInterval - (Date.now() - lastApiRequestAt);
    ...
    async function runBatch(rowsToProcess, fields, processor = processRow) {
        ...
        while (round <= MAX_RETRY_ROUNDS && roundRows.length && state.running) {
            const total = roundRows.length;
            const retryRows = [];
            finalRetryRows = retryRows;
            bar.style.width = '0';
            ...
            for (let i = 0; i < total; i++) {
                ...
                try {
                    ...
                    if (isAuthError(e)) {
                        ...
                        row.status = 'fail';
                        row.note = round < MAX_RETRY_ROUNDS ? '临时风控，第一遍失败，完成后自动重试' : '临时风控，自动重试后仍失败';
                        retryRows.push(row);
                    } else {
                        row.status = 'fail';
                        row.note = e.message;
            ...
            if (retryRows.length && round < MAX_RETRY_ROUNDS) {
                statusEl.textContent = `第${round}遍完成，失败 ${retryRows.length} 个，开始第${round + 1}遍自动重试`;
        ...
        if (!stopped && state.running) {
            finalRetryRows.forEach(row => {
                row.status = 'fail';
                if (!row.note || row.note === '等待自动重试') row.note = '自动重试后仍失败';
            });
            renderBatchOutput();
            const failedCount = rowsToProcess.filter(row => row.status === 'fail').length;
            if (finalRetryRows.length) {
                statusEl.textContent = `自动查询完成，自动重试后仍有 ${finalRetryRows.length} 个申请号失败，请点击“重查失败项”`;
            } else if (failedCount) {
